...
class StretchedGridPoisson2D:
    ...
    def solvePoisson(self, numOfIt, debug=False, dbFilename="./debug.npz"):
        ...
        for i in range(0,numOfIt):
            ...
            if self.neumTop:
                self.soln[:,0] = self.soln[:,-2]-self.dY[-1]*self.neumBotH(self.x,self.y[-1],self.soln[:,-1],self.f[:,-1])
            if self.neumLef:
                self.soln[0,:] = self.soln[1,:]-self.dX[0]*self.neumLefH(self.x[0],self.y,self.soln[0,:],self.f[0,:])
            if self.neumRig:
                self.soln[-1:] = self.soln[-2,:]-self.dX[-1]*self.neumLefH(self.x[-1],self.y,self.soln[,:],self.f[-1,:])
